...
- **Tools image** (`openclaw:tools`): built from `Dockerfile.tools` here, extends base with Claude Code, Kimi CLI, OpenCode, gh, qmd, Homebrew
...
- qmd installed via `npm install -g` (not bun — bun's module resolution breaks node)
